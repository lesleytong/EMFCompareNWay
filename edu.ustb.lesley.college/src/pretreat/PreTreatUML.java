...
public class PreTreatUML {
	...
	public static void main(String[] args) {
		...
		URI originURI = URI
				.createFileURI("E:\\eclipse-dsl-workspace\\edu.ustb.lesley.testuml\\src\\pretreat\\JSON_origin.uml");

		URI baseURI = URI.createFileURI("E:\\eclipse-dsl-workspace\\edu.ustb.lesley.testuml\\src\\pretreat\\JSON.uml");
	...
	public static void extractType(HashMap<Object, Object> internalTypeMap, EObject model) {
		...
		model.eAllContents().forEachRemaining(c -> {
			if (c instanceof PackageImport) {	// for GenMyModel's .uml
				EcoreUtil.remove(c);
		...
		pType.forEach(type -> {
			...
			((ModelImpl) model).getOwnedTypes().add(createPrimitiveType);
		});
